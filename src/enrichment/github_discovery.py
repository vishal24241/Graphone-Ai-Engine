...
async def main():
    ...
    # Optional test limit; normal production run remains unchanged.
    test_limit = os.getenv("GITHUB_TEST_LIMIT")
    if test_limit:
        try:
            limit = max(1, int(test_limit))
            papers = papers[:limit]
            print(f"TEST MODE: processing only {len(papers)} papers")
        except ValueError:
            print("WARNING: Invalid GITHUB_TEST_LIMIT; processing all papers.")
